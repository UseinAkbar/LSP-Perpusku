...
@section('content')
    ...
    <div class="card container-fluid mb-3 py-5">

        <div class="row d-flex flex-wrap justify-content-center">

            @forelse ($buku as $item)
                <div class="col-auto my-2" style="width:18rem;">
                    <div class="card mx-2 my-2 border" style="min-height:28rem;">
                        @if ($item->gambar != null)
                            <img class="card-img-top" style="max-height:180px;object-fit:cover;" src="{{ asset('/images/' . $item->gambar) }}">
                        @else
                            <img class="card-img-top" style="height:200px;object-fit:cover;" src="{{ asset('/images/default.jpg') }}">
                        @endif
                        <div class="card-body d-flex flex-column justify-content-between">
                            <div class="detai-buku">
                                <h5 class="card-title text-primary"><a
                                        href="/buku/{{ $item->id }}"style="text-decoration: none; font-size:1rem;font-weight:bold;color: #1F4172">
                                        {{ $item->judul }}</a></h5>
                                <p class = "cart-text m-0">Kode Buku : <span class='text-primary' style="font-weight:bold;">{{ $item->kode_buku }}</span></p>
                                <p class="card-text m-0">
                                    Pengarang : <span class='text-primary' style="font-weight:bold;">{{$item->pengarang }}</span></p>
            ...
            @empty
                <h3 class="text-primary mt-3">Data buku tidak ada</h3>
            @endforelse
